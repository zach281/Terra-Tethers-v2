...
async function getLeaderboard(): Promise<LeaderboardEntry[]> {
  try {
    ...
    for (const h of (holdings ?? []) as any[]) {
      const price = (h.coin as { current_price: number } | null)?.current_price ?? 0
      holdingsMap[h.user_id] = (holdingsMap[h.user_id] ?? 0) + h.quantity * price
    }
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-zinc-800/60">
          {leaderboard.map(entry => {
            ...
            return (
              <div key={entry.user_id} className={cn('grid grid-cols-[48px_1fr_auto_auto_auto] gap-4 px-4 py-3 items-center transition-colors', isMe ? 'bg-white/5' : 'hover:bg-white/2')}>
                <div className="flex items-center justify-center">
                  {RANK_ICONS[entry.rank] ?? <span className="text-sm font-bold text-zinc-500">#{entry.rank}</span>}
                </div>
                ...
                <div className="text-sm text-zinc-500 text-right hidden sm:block">{entry.total_trades}</div>
                ...
                <div className="text-sm font-bold text-white text-right">{formatCurrency(entry.portfolio_value)}</div>
              </div>
            )
          })}
        </div>
